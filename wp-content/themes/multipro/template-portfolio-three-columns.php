			...
			<div id="portfolio-content" class="clearfix">
				...
				<?php if ($query->have_posts()) : while ($query->have_posts()) : $query->the_post(); ?>
					...
					<li data-id="id-<?php echo $count; ?>" data-type="<?php foreach ($terms as $term) { echo strtolower(preg_replace('/\s+/', '-', $term->name)). ' '; } ?>" class="project clearfix">
						<!-- BEGIN .post -->
						<div <?php post_class('portfolio-three-item'); ?> id="post-<?php the_ID(); ?>">
							<!-- BEGIN .post-content -->
							<div class="post-content clearfix">
								...
								<?php if( has_post_thumbnail() && of_get_option('enable_lightbox') ) { ?>
									...
									<?php if( $video['vimeo-youtube'] != '' || $video['embedded-code'] != '' ){ ?>
										...
										<?php video_lightbox(get_the_ID(), 280, 140); ?>
									<?php }else{ ?>
										<a href="<?php echo get_thumbnail_url(); ?>" title="<?php the_title(); ?>" rel="prettyPhoto[gallery]"><?php resize_img("width=280&height=140"); ?></a>
									<?php } ?>
									</div>
								<?php }elseif( has_post_thumbnail() ){ ?>
									<div class="post-thumb">
										<a href="<?php the_permalink(); ?>" title="<?php the_title(); ?>" ><?php resize_img("width=280&height=140"); ?></a>
									</div>
								<?php } ?>
