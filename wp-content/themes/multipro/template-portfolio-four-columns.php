		...
		<div id="portfolio-wrap" class="clearfix">		
			...
			<div id="portfolio-content" class="clearfix">
				...
				<li data-id="id-<?php echo $count; ?>" data-type="<?php foreach ($terms as $term) { echo strtolower(preg_replace('/\s+/', '-', $term->name)). ' '; } ?>" class="project clearfix">
					<!-- BEGIN .post -->
					<div <?php post_class('portfolio-four-item'); ?> id="post-<?php the_ID(); ?>">
						<!-- BEGIN .post-content -->
						<div class="post-content clearfix">
							...
							<?php if( has_post_thumbnail() && of_get_option('enable_lightbox') ) { ?>
								...
								<?php video_lightbox(get_the_ID(), 205, 140); ?>
								<?php }else{ ?>
									<a href="<?php echo get_thumbnail_url(); ?>" title="<?php the_title(); ?>" rel="prettyPhoto[gallery]"><?php resize_img("width=205&height=140"); ?></a>
								<?php } ?>
									</div>
								<?php }elseif( has_post_thumbnail() ){ ?>
									<div class="post-thumb">
										<a href="<?php the_permalink(); ?>" title="<?php the_title(); ?>" ><?php resize_img("width=205&height=140"); ?></a>
									</div>
								<?php } ?>
